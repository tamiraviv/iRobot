...
Direction _313178576_C::step(Direction prevStep)
{
	...
	// - is not recharging atm, should not go back to the docking station, and should pick a new step
		...
		if (si.dirtLevel >= 1)
			...
			if (house.hasCell(west) && (house.getCell(west) == 'W') && house.hasCell(south) && (house.getCell(south) == 'X')) {
			...
			else if (house.hasCell(east) && (house.getCell(east) == 'W') && house.hasCell(south) && (house.getCell(south) == 'X')) {
				step = Direction::South;
			}
